  ...
  try {
    if(action==='setup-import'){
      ...
      return store.transaction(()=>{
        ...
        if(prior){
          const {verified_bundle_sha256,verified_members,operator_id:previousOperator,...original}=JSON.parse(prior.payload_json);
        ...
        store.db.prepare("INSERT INTO ow_setup_receipts VALUES(?,?,?,'HISTORICAL_SETUP_NOT_APPROVAL')").run(key,hash,JSON.stringify({...payload.receipt,verified_bundle_sha256:payload.bundle_sha256,verified_members:payload.verified_members,operator_id}));
