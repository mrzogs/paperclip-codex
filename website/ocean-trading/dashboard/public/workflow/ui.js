...
function signIn(message = '') {
  ...
  content.innerHTML = `<form class='login' id='login'><h1>Sign in to Ocean</h1><p class='muted'>Wayne's workflow account</p><div class='field'><label for='credential'>Password</label><input id='credential' type='password' autocomplete='current-password' required maxlength='256' spellcheck='false'></div><p class='form-error' role='alert'>${message ? esc(message) : ''}</p><button class='primary' type='submit'>Sign in</button><details class='access-help'><summary>Set up or recover access</summary><p>Run the protected local operator command in your Windows PowerShell window. Enter your password there, never in chat.</p><code>&amp; 'D:\\Paperclip-codex\\scripts\\ocean-workflow-operator.ps1' -Action Initialize</code><p>For an existing account, use <code>-Action Reset-Password</code>. Then restart the website:</p><code>&amp; 'D:\\Paperclip-codex\\scripts\\start-ocean-trading-stack.ps1' -WebsiteOnly -RestartWebsite</code><p>Access setup does not approve a strategy or start trading.</p></details></form>`;
